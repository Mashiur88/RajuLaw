                        ...
                        @foreach ($fetch_data as $key => $data)
                            <div class="list-group">
                                ...
                                @foreach ($data as $data2)
                                    <a href="javascript:void(0);"
                                        class="list-group-item list-group-item-action">{{ $data2->subject }} <button
                    ...
                    @if ($update_mode)
                        <div class="card mb-6">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <h5 class="mb-0">{{ $update_mode ? 'update' : 'Add' }} {{ $subject }}</h5>
                            </div>
                            <div class="card-body">
                                <form wire:submit.prevent='{{ $update_mode ? 'update' : 'store' }}'>
                                    <x-form.input_field labelname="Text" for="text"
                                        wire:model.debounce.300ms='text' />
